...
impl<K, V> Node<K, V> {
    ...
    pub fn is_empty(&self, branches: &Shared<Branch<K, V>>) -> bool {
        match &self.handle {
            DataHandle::Empty => true,
            DataHandle::Leaf(_) | DataHandle::Full { .. } => false,
            DataHandle::Branch(branch) => branch.get(branches).is_empty(),
        }
    }
    ...
    where
        ...
        V: Debug,
    {
        struct NodeDebug<'a, K, V>(&'a Node<K, V>, &'a Trie<K, V>);
        impl<'a, K: Debug, V: Debug> Debug for NodeDebug<'a, K, V> {
            fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
                let mut f = f.debug_struct(match self.0.handle {
                ...
                if !self.0.prefix().is_empty() {
                    f.field("prefix", &self.0.prefix);
                }
                if let Some(leaf) = self.0.leaf() {
                    f.field("leaf", &leaf.get(&self.1.leaves));
                }
                if let Some(branch) = self.0.branch() {
                    f.field("branch", &branch.get(&self.1.branches).branch_debug(self.1));
                }
